                            ...
                            <div class="ps-tab active">
                                <div class="ps-block__item">
                                    @forelse (brands(16) as $brand)
                                        ...
                                        @if (empty($brand->logo))
                                            <img src="{{ asset('vendor-store.jpg') }}" class="rounded w-15 text-center"
                                                 alt="#{{ $brand->name }}">
                                        @else
                                            <img src="{{ filePath($brand->logo) }}" class="rounded w-15 text-center"
                                                 alt="#{{ $brand->name }}">
                                        @endif
                                    ...
                                    @foreach ($sale_items->sale_products as $sale_item)
                                        <div class="ps-product">
                                            <div class="ps-product__thumbnail">
                                                ...
                                                <div class="ps-product__badge">{{formatPrice(brandProductSalePrice($sale_item->sellers)->min())}}
                                                    - {{formatPrice(brandProductSalePrice($sale_item->sellers)->max())}}</div>
                                            </div>
                                            <div class="ps-product__container">
                                                <div class="ps-product__content"><a class="ps-product__title"
                                                                                    href="{{ route('single.product',[$sale_item->sku,$sale_item->slug]) }}">{{ $sale_item->name }}</a>

                                                    {{formatPrice(brandProductPrice($sale_item->sellers)->min()) == formatPrice(brandProductPrice($sale_item->sellers)->max()) ? formatPrice(brandProductPrice($sale_item->sellers)->min()) : formatPrice(brandProductPrice($sale_item->sellers)->min()) . '-' . formatPrice(brandProductPrice($sale_item->sellers)->max())  }}
