...
@section('content')
    <div class="card m-2">
        <div class="card-header">
            ...
            <div class="float-right">
                <div class="row text-right">
                    <div class="col-12">
                        <form action="{{route('seller.campaign.products.search')}}" method="get">
                            @csrf
        ...
        <div class="card-body">
            ...
            <input type="hidden" class="campaign-list-url" value="{{route('seller.campaign.products.index', $campaign->slug)}}">
            <div class="row mt-3 justify-content-center">
                @forelse($products_list as $product)
                    @if($campaign_products->where('vendor_product_id',$product->id)->count()>0)
                        @if($campaign_products->where('vendor_product_id',$product->id)->first()->campaign_id == $campaign->id)
                            <div class="card col-md-2 mx-3 p-0" title="{{$product->name}}">
                                <div class="card-header border-bottom-0 text-center pb-0 text-truncate">
                                    {{$product->name}}
                                </div>
                                <div class="card-body">
                                    <img src="{{$product->img}}" alt="image" class="img-fluid rounded-sm"/>
                                    <div class="text-center text-bold mt-2">
                                        {{$product->price}}
                                    </div>
                                </div>

                                @if(!is_null($campaign_products) && !is_null($campaign_products->where('vendor_product_id', $product->id)->first()))
                                    <div class="card-footer text-center campaign-add-btn-{{$product->id}}">
                                        <a href="#!"
                                           class="btn btn-danger px-3"
                                           onclick="removeFromCampaign({{$product->id}},{{$campaign->id}})">@translate(Remove)</a>
                                    </div>
                                @else
                                    <div class="card-footer text-center campaign-add-btn-{{$product->id}}">
                                        <a href="#!"
                                           class="btn btn-success px-3"
                                           onclick="addToCampaign({{$product->id}},{{$campaign->id}})">@translate(Add To Campaign)</a>
                                    </div>
                                @endif
                            </div>
                            @endif
