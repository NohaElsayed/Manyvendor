...
@section('title') @translate(Admins Earning)
@endsection
    ...
    <div class="card m-2">
        <div class="card-header">
            <div class="float-left">
                <h2 class="card-title">@translate(Earning list)</h2>
            </div>
            ...
            <table class="table table-striped table-bordered table-hover">
                ...
                @forelse($earning as  $item)
                    <tr>
                        ...
                        <td class="text-left">#{{$item->booking_code}}</td>
                        <td class="text-left">{{$item->user->name}}</td>
                        <td class="text-left">{{formatPrice($item->price)}}</td>
                        <td class="text-left">{{$item->percentage}}</td>
                        <td class="text-left">{{formatPrice($item->commission)}}</td>
                        <td class="text-left">{{date('d-M-Y',strtotime($item->created_at))}}</td>
                    </tr>
                @empty
                    <tr>
