<div class="card-body">
    <form action="{{ route('header.promotion.update',$header_banner_edit->id) }}" method="post"
        ...
        <div class="form-group">
            ...
            <select class="form-control select2" name="category_id">
                ...
                @foreach($categories as $category)
                    <option
                        value="{{$category->id}}" {{ $header_banner_edit->category_id == $category->id ? 'selected' : '' }}>{{$category->name}}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input class="form-control" name="link" value="{{ $header_banner_edit->link }}"
        ...
        <div class="form-group">
            ...
            <div class="">
                <input name="oldImage" type="hidden" value="{{ $header_banner_edit->image }}">
                <img src="{{ filePath($header_banner_edit->image) }}" alt="#Main Slider">
            </div>
        </div>
        ...
        <div class="form-group">
            <label> @translate(Publish?)
                <input class="add-variant" type="checkbox"
                       name="is_published" {{ $header_banner_edit->is_published == 1 ? 'checked' : ''}}>
            </label>
        </div>
