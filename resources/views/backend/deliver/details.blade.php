<form action="{{route('deliver.confirm')}}" method="post">
    @csrf
    ...
    <div class="card-body">
        <h5 class="card-title">
            ...
            <p>@translate(First Name) : <span class="text-info">  {{$user->first_name}}</span>
        ...
        <div>
            <button name="status" value="Confirm" class="btn btn-success float-right">Confirm</button>
            <button name="status" value="Cancel" class="btn btn-outline-danger float-left">Cancel</button>
        </div>
    </div>
</form>
