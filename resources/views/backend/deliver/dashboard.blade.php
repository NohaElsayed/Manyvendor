...
@section('title') Assign Orders @endsection
        ...
        <div class="card card-primary card-outline">
            <div class="card-body p-2 table-responsive">
                <table class="table table-bordered border" id="example">
                    ...
                    @forelse ($assignOrder as $getOrder)

                        @if($getOrder->orderDetails->status == 'confirmed')
                            <tr>
                                ...
                                <td class="w-20">
                                    <span class="d-block">
                                        @translate(Name) - {{ $getOrder->orderDetails->user ? $getOrder->orderDetails->user->name: 'Guest Order' }}
                                    </span>
                                    <span class="d-block">
                                ...
                                <td class="w-15">
                                    <span class="d-block">
                                        @if (isset($getOrder->orderDetails->comment) || isset($getOrder->orderDetails->commentedBy))
                                            ...
                                            By {{ $getOrder->orderDetails->commentedBy->name ?? 'N/A' }}
                                        @else
                                ...
                                <td>
                                    @if ($getOrder->orderDetails->status != 'delivered')
                                        @if($getOrder->pick)

                                            <a onclick="forModal('{{ route('order.location.status', $getOrder->id) }}','@translate(Update location)')"
                                               href="javascript:void(0)"
                                               class="btn-sm btn-dark d-block m-2 text-center">@translate(Update Location)</a>

                                            <a href="javascript:void(0)"
                                               onclick="forModal('{{route('deliver.order.followup', $getOrder->orderDetails->id)}}','@translate(Follow up comment)')"
                                               class="btn-sm btn-info d-block m-2 text-center">@translate(Follow Up)</a>
                                            <hr>
                                            <a href="{{ route('deliver.order.delivered', $getOrder->orderDetails->id) }}"
                                               class="btn-sm btn-delivered d-block m-2 text-center">@translate(Delivered)</a>


                                        @else
                                            <a href="javascript:void(0)"
                                               onclick="forModal('{{ route('deliver.pick', $getOrder->id) }}','@translate(Pick The Order)')"
                                               class="btn-sm btn-delivered d-block m-2 text-center">@translate(Pick The
                                                Order)</a>
                                        @endif
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if ($getOrder->orderDetails->status != 'delivered')
                                        @if($getOrder->pick)
                                            ...
                                            <a href="javascript:void(0)"
                                               onclick="forModal('{{route('order.followup', $getOrder->orderDetails->id)}}','@translate(Follow up comment)')"
                                               class="btn-sm btn-info d-block m-2 text-center">@translate(Follow Up)</a>
                                            <hr>
                                            <a href="{{ route('order.delivered', $getOrder->orderDetails->id) }}"
                                               class="btn-sm btn-delivered d-block m-2 text-center">@translate(Delivered)</a>
